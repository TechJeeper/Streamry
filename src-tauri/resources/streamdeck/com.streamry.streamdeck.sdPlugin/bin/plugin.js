...
function loadConnection() {
    const file = path__namespace.join(pluginRoot(), "streamry-connection.json");
    try {
        if (fs__namespace.existsSync(file)) {
            const raw = JSON.parse(fs__namespace.readFileSync(file, "utf8"));
            return {
...
async function runOrAlert(ev, fn) {
    try {
        await fn();
    }
    catch {
...
let RunCommandAction = (() => {
    let _classDecorators = [action({ UUID: "com.streamry.streamdeck.command.run" })];
...
let RunAutomationAction = (() => {
    let _classDecorators = [action({ UUID: "com.streamry.streamdeck.automation.run" })];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = SingletonAction;
...
let StartGiveawayAction = (() => {
    let _classDecorators = [action({ UUID: "com.streamry.streamdeck.giveaway.start" })];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = SingletonAction;
...
let StopGiveawayAction = (() => {
    let _classDecorators = [action({ UUID: "com.streamry.streamdeck.giveaway.stop" })];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = SingletonAction;
...
let DrawGiveawayAction = (() => {
    let _classDecorators = [action({ UUID: "com.streamry.streamdeck.giveaway.draw" })];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = SingletonAction;
...
let PlayMediaAction = (() => {
    let _classDecorators = [action({ UUID: "com.streamry.streamdeck.media.play" })];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = SingletonAction;
...
let SendChatAction = (() => {
    let _classDecorators = [action({ UUID: "com.streamry.streamdeck.chat.send" })];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = SingletonAction;
    (class extends _classSuper {
        ...
        async onWillAppear(ev) {
            ...
            if (msg)
                await ev.action.setTitle(msg.length > 12 ? `${msg.slice(0, 12)}…` : msg);
        }
        async onKeyDown(ev) {
...
let ConnectBotAction = (() => {
    let _classDecorators = [action({ UUID: "com.streamry.streamdeck.bot.connect" })];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = SingletonAction;
...
let DisconnectBotAction = (() => {
    let _classDecorators = [action({ UUID: "com.streamry.streamdeck.bot.disconnect" })];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = SingletonAction;
...
streamDeck.ui.onSendToPlugin(async (ev) => {
    ...
    if (payload?.event !== "list" || !payload.kind)
        return;
    try {
        const path = payload.kind === "commands"
            ? "/api/v1/commands"
            : payload.kind === "automations"
                ...
                : payload.kind === "giveaways"
                    ? "/api/v1/giveaways"
                    : "/api/v1/media";
        const items = await apiJson("GET", path);
